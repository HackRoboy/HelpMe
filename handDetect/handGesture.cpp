...
void HandGesture::addNumberToImg(MyImage *m){
	...
	for(int i=0;i<numbers2Display.size();i++){
        rectangle(m->src,Point(xPos,yPos),Point(xPos+offset+100,yPos+offset),numberColor, 2);
        if(numbers2Display[i]==5)
        {
            //putText(m->src, intToString(numbers2Display[i]),Point(xPos+7,yPos+offset-3),fontFace,fontSize,numberColor);
            putText(m->src, "Emergency!!",Point(xPos+7,yPos+offset-3),fontFace,fontSize,numberColor);
            xPos+=200;
...
int HandGesture::getNrFingerTips()
{

    return fingerTips.size();
}

int HandGesture::getDefectsNr()
{
    return defects.size();
}
