...
void extractForeground(const Mat &img, Mat &foreGround, BackgroundSubtractorMOG2 &mog) {
...
void makeContours(MyImage *m, HandGesture* hg, BackgroundSubtractorMOG2 &mog, Mat fg){
    Mat aBw;
    pyrUp(m->bw,m->bw);
    m->bw.copyTo(aBw);
...
//        cout << "fingers" << hg.getNrFingerTips() << endl;
//        cout << "Defects" << hg.getDefectsNr() << endl;
